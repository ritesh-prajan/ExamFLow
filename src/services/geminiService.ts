...
export function getApiKey(): string | undefined {
  ...
  if (envKey && envKey !== '[ADD_YOUR_GEMINI_API_KEY_HERE]') {
    return envKey;
  }
  return undefined;
}
...
export async function parseSyllabus(text: string): Promise<{ subject: string; subjectCode: string; modules: Module[]; topics: Topic[] }> {
  try {
    ...
    if (!getApiKey()) {
      throw new Error("Gemini API key is not configured. Please configure it in Settings or add GEMINI_API_KEY to your environment variables.");
    }
