...
import { parseSyllabus, estimateTopicDurations, getApiKey } from '@/services/geminiService';
import { getDriveSearchUrl } from '@/lib/driveUtils';
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.subjectCode && !subjectCode) {
        ...
        // If we found a code and don't have a notes URL yet, suggest a search URL
        if (!notesUrl) {
          setNotesUrl(getDriveSearchUrl(result.subject || name, result.subjectCode));
        }
      } else if (result.subject && !notesUrl) {
        setNotesUrl(getDriveSearchUrl(result.subject));
      }
          ...
          >
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col overflow-hidden relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-muted-foreground">Subject Code (Optional)</label>
                  <input
                    type="text"
                    value={subjectCode}
                ...
                <div className="md:col-span-2 space-y-2">
                  <label className="text-sm font-medium text-muted-foreground">Notes/Drive Link (Optional)</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-accent border border-border rounded-xl pl-12 pr-24 py-3 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all"
                    />
                    {name && (
                      <button
                        type="button"
                        onClick={() => setNotesUrl(getDriveSearchUrl(name, subjectCode))}
                        className="absolute right-2 top-1/2 -translate-y-1/2 px-3 py-1.5 bg-secondary text-secondary-foreground text-[10px] font-bold uppercase rounded-lg hover:bg-secondary/80 transition-all flex items-center gap-1"
                      >
                        <SearchIcon size={12} />
                        Auto-Find
                      </button>
                  ...
                  <p className="text-[10px] text-muted-foreground px-1">Paste your personalized notes folder link here.</p>
                  <div className="flex flex-wrap gap-2 mt-2 px-1">
                    <span className="text-[10px] font-bold text-muted-foreground uppercase opacity-50 flex items-center gap-1 mt-1">
                      Browse Libraries:
                    </span>
                    <a
                      href="https://drive.google.com/drive/folders/1TUL5IYTnipICLSvu4bzpPGuU97z15FfC?usp=sharing"
                      target="_blank"
                      rel="noreferrer"
                    ...
                    <a
                      href="https://drive.google.com/drive/folders/1Z4tBts_Y55n4m8yRSyV7WzKocVHpi9yC?usp=sharing"
                      target="_blank"
                      rel="noreferrer"
                      className="text-[10px] font-bold text-primary hover:underline bg-primary/5 px-2 py-1 rounded"
                    >
