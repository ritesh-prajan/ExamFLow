...
export default function FocusMode() {
  ...
  useEffect(() => {
    if (currentTopic && state.selectedSubjectId && state.user?.uid) {
      localStorage.setItem(`${state.user.uid}_activeModule_${state.selectedSubjectId}`, currentTopic.module);
      // Sync moduleId for Dashboard dropdown as well
      const moduleId = currentTopic.module.toLowerCase().replace(/\s+/g, '-');
      localStorage.setItem(`${state.user.uid}_activeModuleId_${state.selectedSubjectId}`, moduleId);
    }
  }, [currentTopic, state.selectedSubjectId, state.user?.uid]);
  ...
  if (!currentTopic && (state.topics || []).length > 0 && state.selectedSubjectId) {
    if (state.learningPreference === 'sequential') {
      const activeModuleName = localStorage.getItem(`${state.user?.uid || 'anonymous'}_activeModule_${state.selectedSubjectId}`);
      ...
      if (!currentTopic) {
        const activeModuleId = localStorage.getItem(`${state.user?.uid || 'anonymous'}_activeModuleId_${state.selectedSubjectId}`);
        ...
        if (targetModule) {
          ...
          if (currentTopic && state.user?.uid) {
            localStorage.setItem(`${state.user.uid}_activeModule_${state.selectedSubjectId}`, currentTopic.module);
            const mid = targetModule.id;
            localStorage.setItem(`${state.user.uid}_activeModuleId_${state.selectedSubjectId}`, mid);
          }
        }
      }
    } else {
  ...
  const handleComplete = async () => {
    ...
    if (isTimerEnabled && isTimerRunning) {
      logTopicTime(currentTopic.id, timerElapsedTime);
    }
    ...
    if (state.learningPreference === 'sequential') {
      ...
      if (moduleFinished) {
        ...
        if (nextIncomplete && state.selectedSubjectId && state.user?.uid) {
      ...
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
        ...
        <div className="lg:col-span-8 flex flex-col h-full">
          <motion.div
            ...
            className="flex-1 bg-card hover:bg-card/90 backdrop-blur-3xl border border-border rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 md:p-16 shadow-2xl relative overflow-hidden flex flex-col justify-center transition-colors lg:min-h-[500px]"
          >
            <div className="absolute top-0 left-1/2 -translate-x-1/2 w-24 sm:w-48 h-1 bg-primary/20 rounded-full mt-4 sm:mt-6" />
            
            <div className="flex flex-col items-center text-center">
              <div className="inline-flex items-center gap-2 px-3 py-1 sm:px-4 sm:py-1.5 bg-primary/10 rounded-full border border-primary/20 text-primary text-[8px] sm:text-[10px] font-bold uppercase tracking-[0.4em] mb-4 sm:mb-8">
                <Brain size={14} className="animate-pulse" />
                {currentTopic.module}
              </div>
              
              <h1 className="text-2xl sm:text-5xl md:text-7xl font-black mb-6 sm:mb-10 leading-[1.1] tracking-tight text-foreground max-w-4xl px-2 sm:px-0">
                {currentTopic.name}
              </h1>
              
              <div className="w-full max-w-md mx-auto space-y-3 sm:space-y-6 text-left">
                <h3 className="text-muted-foreground/30 text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.25em] mb-2 sm:mb-4 flex items-center gap-2 sm:gap-3">
                  <span className="w-6 sm:w-8 h-[1px] bg-border/50"></span>
                  Active Protocol
                ...
                {[
                  ...
                  <motion.div 
                    ...
                    className="flex gap-3 sm:gap-5 items-start bg-accent/30 p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-border/10 shadow-sm"
                  >
                    <div className="w-4 h-4 sm:w-6 sm:h-6 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center shrink-0 mt-1 shadow-[0_0_15px_rgba(var(--primary),0.2)]">
                      <div className="w-1.5 h-1.5 rounded-full bg-primary shadow-glow" />
                    </div>
                    <p className="text-xs sm:text-base md:text-xl text-muted-foreground font-medium leading-relaxed">{point}</p>
                  </motion.div>
                ))}
              </div>
            </div>
