...
import { Plus, BookOpen, Calendar, Clock, ChevronRight, LayoutGrid, List, Trash2, Archive, Activity, Pencil, FileText } from 'lucide-react';
              ...
              <div className="flex flex-wrap items-center gap-2">
                {subject.notesUrl && (
                  <a
                    ...
                    className="p-2 bg-primary/10 rounded-lg text-primary hover:bg-primary/20 transition-all border border-primary/20"
                    title="Open Notes"
                  >
                    <FileText size={18} />
                  </a>
                ...
                <button
                  ...
                  className="p-2 bg-accent rounded-lg text-muted-foreground hover:bg-primary/10 hover:text-primary transition-all border border-border"
                  title="Edit Subject"
                >
                  <Pencil size={18} />
                </button>
                <button
                  ...
                  className="p-2 bg-accent rounded-lg text-muted-foreground hover:bg-destructive/10 hover:text-destructive transition-all border border-border"
                  title="Delete Subject"
                >
                  <Trash2 size={18} />
                </button>
                <div className="p-2 bg-accent rounded-lg group-hover:bg-primary group-hover:text-primary-foreground transition-all border border-border">
  ...
  return (
    <div className="min-h-screen pt-20 sm:pt-24 pb-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div>
          <h1 className="text-3xl sm:text-4xl font-black mb-2 uppercase tracking-tight">My Subjects</h1>
          <p className="text-muted-foreground text-xs sm:text-sm font-medium">Select a subject to continue your preparation</p>
        </div>
        
        <div className="flex flex-wrap items-center gap-3 sm:gap-4 overflow-x-auto pb-2 sm:pb-0 no-scrollbar">
          <div className="flex bg-accent/30 p-1 rounded-xl border border-border">
            <button
              onClick={() => setViewMode('grid')}
              className={`p-2 rounded-lg transition-all ${viewMode === 'grid' ? 'bg-primary text-primary-foreground shadow-lg' : 'text-muted-foreground hover:text-foreground'}`}
            >
              <LayoutGrid size={20} />
            </button>
            <button
          ...
          <button
            ...
            className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-6 py-4 bg-primary text-primary-foreground font-black uppercase tracking-widest rounded-xl hover:bg-primary/90 transition-all shadow-xl shadow-primary/20 text-[10px]"
          >
            <Plus size={20} />
