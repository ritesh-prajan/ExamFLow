  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4 sm:px-6 relative">
        <div className="absolute top-8 sm:top-12 left-4 sm:left-12">
          <Link to="/" className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition-colors text-xs sm:text-sm font-medium">
            <ArrowLeft size={16} />
            Back to Site
          </Link>
        </div>
        <div className="glass p-6 sm:p-10 w-full max-w-md text-center">
          <div className="w-14 h-14 sm:w-16 sm:h-16 bg-secondary/20 rounded-2xl flex items-center justify-center mx-auto mb-6">
            <Lock className="text-secondary" size={28} />
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold mb-2 text-foreground">Professor Portal</h1>
          <p className="text-sm text-muted-foreground mb-8">Access faculty-only subject configuration tools.</p>
  ...
  return (
    <div className="min-h-screen pt-20 sm:pt-24 pb-12 px-4 sm:px-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8 sm:mb-12">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold mb-1 text-foreground uppercase tracking-tight">Faculty Dashboard</h1>
          <p className="text-sm text-muted-foreground">Managing 3 active subjects.</p>
        </div>
        <button className="w-full md:w-auto flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-secondary text-white font-bold hover:bg-secondary/90 transition-all active:scale-95 shadow-lg shadow-secondary/20 btn-touch text-sm sm:text-base">
        ...
        <div className="lg:col-span-8 space-y-8">
          <div className="glass p-8">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-10 sm:mb-12">
              <div className="text-center p-4 rounded-2xl bg-accent/20">
                <div className="text-2xl sm:text-3xl font-bold mb-1 text-foreground">74%</div>
                <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">Avg. Coverage</div>
              </div>
              <div className="text-center p-4 rounded-2xl bg-accent/20">
                <div className="text-2xl sm:text-3xl font-bold mb-1 text-foreground">62%</div>
                <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">Avg. Mastery</div>
              </div>
              <div className="text-center p-4 rounded-2xl bg-destructive/10">
                <div className="text-2xl sm:text-3xl font-bold mb-1 text-destructive">12%</div>
                <div className="text-[10px] text-muted-foreground font-bold uppercase tracking-widest">At Risk</div>
              </div>
            </div>
