...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen pt-16">
      {/* Hero Section */}
      <section className="px-6 py-16 sm:py-24 max-w-7xl mx-auto text-center">
        <motion.h1 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          className="text-4xl sm:text-5xl md:text-7xl font-black tracking-tight mb-6 uppercase"
        >
          Stop deciding what to study.<br />
          <span className="text-primary shadow-glow">Start studying.</span>
        </motion.h1>
        <motion.p 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.1 }}
          className="text-lg sm:text-xl text-muted-foreground max-w-2xl mx-auto mb-10 font-medium"
        >
      ...
      <footer className="px-6 py-12 border-t border-border text-center text-muted-foreground">
        ...
        <p className="mb-6">ExamFlow · Study Smarter · Ritesh Prajan S</p>
        <div className="flex justify-center gap-8 text-sm">
          <Link to="/privacy" className="hover:text-foreground transition-colors">Privacy</Link>
          <Link to="/terms" className="hover:text-foreground transition-colors">Terms</Link>
          <Link to="/settings" className="hover:text-foreground transition-colors">Support</Link>
        </div>
      </footer>
    </div>
  );
