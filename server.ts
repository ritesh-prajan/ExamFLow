...
async function startServer() {
  ...
  app.use((req, res, next) => {
    // Only log significant requests to avoid cluttering in the console
    const isAsset = req.url.match(/\.(tsx?|jsx?|css|json|svg|png|jpg|jpeg|gif|webp|woff2?|ttf|otf|map)$/);
    const isVite = req.url.includes('/@vite/') || req.url.includes('/@fs/') || req.url.includes('/@id/');
    const isSource = req.url.startsWith('/src/');
    
    if (!isAsset && !isVite && !isSource && !req.url.includes('/assets/')) {
